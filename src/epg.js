...
export function isoTime(seconds) {
  return new Date((seconds + OFFSET) * 1000).toISOString().slice(0, 19) + '+08:00';
}
...
export function deduplicate(programmes) {
  ...
  for (const programme of programmes) {
    if (!programme || typeof programme.channel !== 'string') continue;
    if (!Number.isSafeInteger(programme.start) || !Number.isSafeInteger(programme.stop)) continue;
...
/**
 * 如果某频道当天首个节目不是 00:00:00（UTC+8），
 * 则用前一天最后一个节目的标题，补一个
 * [当天 00:00:00, 当天首个节目.start - 1] 的节目。
 *
 * 不修改入参数组，返回新增节目列表。
 */
export function fillDayStart(programmes, date) {
  ...
  for (const programme of programmes) {
    if (!programme || typeof programme.channel !== 'string') continue;
    if (!Number.isSafeInteger(programme.start) || !Number.isSafeInteger(programme.stop)) continue;
    if (typeof programme.title !== 'string' || !programme.title) continue;
  ...
  for (const [channel, list] of byChannel) {
    ...
    const previousLast = sorted
      .filter(
        p =>
          p.start >= previousDayStart &&
          p.start < midnight &&
          p.stop <= todayFirst.start
      )
      .sort((a, b) => b.stop - a.stop)[0];

    if (!previousLast) continue;
    if (typeof previousLast.title !== 'string' || !previousLast.title) continue;
...
export async function collect(
  source,
  {
    today = dateKey(Date.now() / 1000),
    pastDays = 3,
    futureDays = 3,
    concurrency = 4,
    minChannels = 50,
    minTodayCoverage = 0.9,
    delayMs = 150,
    onProgress = () => {},
  } = {}
) {
  ...
  if (channels.length < minChannels) {
    throw new Error(`Only ${channels.length} channels; expected at least ${minChannels}`);
  }

  const first = dateKey(range.start);
  const last = dateKey(range.stop - 1);
  ...
  const tasks = channels.flatMap(channel =>
    channel.dates
      .filter(date => date >= firstFetch && date <= last)
      .map(date => ({ channel, date }))
  );
    ...
    Array.from({ length: Math.min(concurrency, tasks.length) }, async () => {
      while (next < tasks.length) {
        ...
        try {
          ...
          programmes.push(
            ...parsed.filter(p => p.start < range.stop && p.stop > range.start)
          );
        } catch (error) {
  ...
  if (failures.length) {
    throw new Error(`${failures.length} schedule request(s) failed:\n${failures.join('\n')}`);
  }

  let result = deduplicate(programmes);
  if (!result.length) {
    throw new Error('No programmes; refusing to publish an empty EPG');
  }

  const filled = fillDayStart(result, today);
  if (filled.length) {
    result = deduplicate([...result, ...filled]);
  }

  const todayChannels = new Set(
    result.filter(p => dateKey(p.start) === today).map(p => p.channel)
  );
  const coverage = todayChannels.size / channels.length;

  if (coverage < minTodayCoverage) {
    throw new Error(
      `Today's channel coverage ${(coverage * 100).toFixed(1)}% is below ${minTodayCoverage * 100}%`
    );
  }
  ...
  const manifest = {
    generatedAt: isoTime(Math.floor(Date.now() / 1000)),
    referenceDate: today,
    source: HOME,
    format: 'XMLTV',
    ...
    programmeCount: result.length,
    channelsWithProgrammes: counts.size,
    todayChannelCoverage: coverage,
    earliestStart: isoTime(result.reduce((min, p) => Math.min(min, p.start), Infinity)),
    latestStop: isoTime(result.reduce((max, p) => Math.max(max, p.stop), -Infinity)),
    requestCount: tasks.length,
    emptySchedules: empty.sort(
      (a, b) => a.channel.localeCompare(b.channel) || a.date.localeCompare(b.date)
    ),
    discardedProgrammes: discarded.sort(
      (a, b) =>
    ...
    missingToday: channels
      .filter(c => !todayChannels.has(c.id))
...
export function renderXml(channels, programmes, { sourceName = '央视频' } = {}) {
  ...
  const tv = doc.ele('tv', {
    'generator-info-name': 'autoEPG',
    'generator-info-url': 'https://github.com/TvWasm/autoEPG',
    'source-info-name': sourceName,
    'source-info-url': HOME,
  });
  ...
  for (const p of [...programmes].sort(
    (a, b) => rank.get(a.channel) - rank.get(b.channel) || a.start - b.start
  )) {
    ...
    tv.ele('programme', {
      ...
      .ele('title', /\p{Script=Han}/u.test(p.title) ? { lang: 'zh' } : {})
      .txt(cleanText(p.title));
  }
...
export async function writeArtifacts(directory, dataset) {
  ...
  for (let day = start; day < stop; day += DAY) {
    ...
    const manifest = {
      ...dataset.manifest,
      date,
      requestedDates: { from: date, to: date },
      programmeCount: programmes.length,
      channelsWithProgrammes: new Set(programmes.map(p => p.channel)).size,
      earliestStart: isoTime(programmes.reduce((min, p) => Math.min(min, p.start), Infinity)),
      latestStop: isoTime(programmes.reduce((max, p) => Math.max(max, p.stop), -Infinity)),
      emptySchedules: dataset.manifest.emptySchedules.filter(s => s.date === date),
      discardedProgrammes: (dataset.manifest.discardedProgrammes ?? []).filter(
        p => dateKey(p.start) === date
      ),
      xmlBytes: xml.length,
    };
    ...
    if (date === dataset.manifest.referenceDate) {
      for (const days of [2, 3]) {
        ...
        manifest.variants.push({
          ...
          from: date,
          to: dateKey(end - 1),
          programmeCount: combined.length,
    ...
    for (const group of GROUPS) {
      ...
      for (const variant of variants) {
        ...
        const subset = dataset.programmes.filter(
          p => ids.has(p.channel) && p.start < day + variant.days * DAY && p.stop > day
        );
      ...
      groups.push({
        id: group.id,
        name: group.name,
        channelIds: members.map(c => c.id),
        feeds,
      });
    }

    files['channels.json'] = Buffer.from(JSON.stringify(dataset.channels, null, 2) + '\n');
    files['groups.json'] = Buffer.from(
      JSON.stringify({ source: '央视频', channelSchemaVersion: 2, groups }, null, 2) + '\n'
    );
